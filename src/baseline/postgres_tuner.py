...
sys.path.insert(0, os.path.join(head, '../')) # /src for access to src/common, src/dqn
...
class PostgresTuner():
    ...
    def act(self, cfg):
        ...
        train_query_sample = np.random.choice(self.train_queries, 
                                              size=max(len(self.train_queries) / 10, 1)) # TODO add to config
...
def main(argv):
    ...
    system_environment = PostgresSystemEnvironment(tables=experiment_config['tables'])
